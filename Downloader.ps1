<# 	
 ...
 .EXAMPLE
	...
	Description
	-----------
	This command uses the Variable.xml and Download.xml files in the C:\PDT directory to define what is downloaded and where the files will be stored.
 ...
 .EXAMPLE
	...
	Description
	-----------
	This command uses the Variable.xml and Download.xml files in the local directory to define what is downloaded and where the files will be stored.
 ...
 .PARAMETER Path
	Specifies the path to the Variable.xml, Download.xml, Installer.ps1, and any Extender files that will be used.
...
{
	try { $Workflow = [XML] (Get-Content "$Path\Download.xml") }
	catch { $Validate = $false; Write-Host " Invalid Download.xml" -ForegroundColor Red }
}
Else
{
	$Validate = $false
	Write-Host " Missing Download.xml" -ForegroundColor Red
}
...
Write-Host
 Write-Host " Identifying download requirements... "
	...
	{
		Write-Host
		Write-Host "  Collecting software information from Download.xml"
